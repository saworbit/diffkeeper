...
DiffKeeper uses kernel-level interception for filesystem writes, enabling sub-microsecond capture latency and <0.5% CPU overhead even on workloads that exceed 10K writes/sec. This guide details how to build, load, and troubleshoot the probes.

## Requirements

- Linux kernel **5.5+** with BTF and eBPF enabled (CONFIG_BPF + CONFIG_BPF_JIT). `fentry` (5.10+) is recommended because write probes rely on it for `bpf_d_path`.
...
1. Generate `vmlinux.h` once per kernel:
   ...
   ```
2. Compile the probes (produces `ebpf/diffkeeper.bpf.o` and refreshes the embedded copy under `pkg/ebpf/`):
   ```bash
   make build-ebpf
   ```
...
|-------|-------------|------|
| `fentry/vfs_write`, `fentry/vfs_writev`, `fentry/vfs_pwritev` | Captures write syscalls, records filename via `dentry->d_name.name` (portable across kernels), emits `syscall_event` structs | `events` (ringbuf) |
...
- Events flow from ring buffers -> Go `pkg/ebpf` manager -> recorder pipeline. The adaptive profiler reprograms filters every `--profiler-interval`.
...
| `invalid argument` when loading probes | Kernel lacks fentry/BTF or rejects helpers like `bpf_d_path` | Use a 5.10+ BTF-enabled kernel, or rely on filename-only capture (default) / fall back to fsnotify |
...
1. Build probes and run stress test:
   ...
   make build
   make build-ebpf
   sudo ./bin/diffkeeper --state-dir=/data --enable-ebpf --profiler-interval=50ms -- nginx -g 'daemon off;'
   ```
2. Generate churn (>10K writes/sec) with `stress-ng --iomix 4`.
3. Watch metrics:
   - `bpftool prog tracelog` for loader errors
   - DiffKeeper logs should report `<1us capture latency` and `<0.5% CPU`
